...
| 05 | How Muscle Contraction Works | 20 | Calcium availability, stimulation frequency, force and sarcomere length, ATP depletion, cross-bridge cycling |

Each module includes an interactive mechanism, glossary, sourced facts, checkpoint questions with explanations and retry, and a completion screen. Muscle Contraction has 17 mechanism chapters and eight checkpoints; modules 01–04 each have four checkpoints. Upcoming topics are labeled as unavailable and are not links.

## Run locally

1. Keep the entire `signal-atlas` folder together.
2. Open `index.html` in a current browser.
3. Choose any of the five live modules.
...
- **Play / Pause:** control biological animation and any time-evolving model in modules 01–04. Muscle Contraction includes guided autoplay (eight seconds per mechanism chapter) and a separate animation pause control.
...
│           ├── crispr.js
│           ├── pressure.js
│           ├── muscle-content.js # Muscle chapters, glossary, references and quiz
│           └── muscle.js         # Muscle simulation and interaction controller
├── topics/
│   ├── how-anesthesia-works.html
│   ├── how-insulin-works.html
│   ├── how-crispr-edits-dna.html
│   ├── blood-pressure-regulation.html
│   └── how-muscle-contraction-works.html
└── docs/
    ├── adding-a-module.md
    └── QA.md
```
...
- **Muscle contraction:** follows mammalian skeletal-muscle excitation–contraction coupling. It distinguishes the local end-plate potential from the muscle action potential, CaV1.1 voltage sensing from RyR1 calcium release, and calcium regulation from the ATP-dependent cross-bridge cycle. Sarcomeres shorten through filament sliding; filament lengths stay constant. Frequency responses and ATP depletion are qualitative teaching models, not predictions of measured force or fatigue.

Scientific references appear in each module’s key facts or completion screen. No third-party images or remote assets are used.

## Extend the atlas

Modules 01–04 register a definition on `window.SignalAtlasModules`. The shared engine reads its chapter data, controls, metric function, scientific SVG renderer, and optional time-step callback. It handles navigation, quizzes, glossary/fact layouts, keyboard controls, persistence, and fullscreen, so additional modules do not need a duplicate application shell.

Muscle Contraction retains its standalone interactive shell and uses the shared visual identity, with content in `muscle-content.js`, a dedicated `muscle.js` controller, and `muscle.css`. It requires no additional dependencies.

Read [Adding a module](docs/adding-a-module.md) for the shared-engine contract and extension workflow. The delivered site contains no unfinished future-topic pages.
...
Development and verification details are in [QA notes](docs/QA.md) and the [Muscle Contraction checks](docs/muscle-qa.md). Development-only tools are not shipped and are not required to use the website.
